...
This project is built with scalablity in mind, based on a microservices architecture and Docker containers.

## Getting Started

Make sure you have [installed](https://docs.docker.com/docker-for-windows/install/) docker in your environment. Clone the repository in your local machine, open up your favourite terminal and then execute the below commands on the root repo directory:
...
- ManagementMicroservice Swagger: http://localhost:45011
- Door EntranceMicroservice Swagger: http://localhost:45012
...
The reason for using microservice oriented architecture was the requirement for the scalablity of the application. These architectural concerns weremind are:

- **The user should NOT wait for more than a few seconds for a door to open**. It fraustrates the people, and cause numerous problems, if commanding a door in front of a person to be opened, only to get errors because of high loads on the backend servers. To address this issue:
